...
static void init_blob(global *all)
{
    int x = all->game.fight[1].pos_x = 1150;
    int y = all->game.fight[1].pos_y = 420;
    char path[] = "./content/perso/sprites_sheets/slime1.png";
    sfVector2f scale = {5, 5};
    sfVector2f pos = {x, y};
    sfIntRect rect = {0, 0, 28, 35};

    iFight[1].texture = sfTexture_createFromFile(path, NULL);
    iFight[1].sprite = sfSprite_create();
    sfSprite_setTexture(iFight[1].sprite, iFight[1].texture, sfTrue);
    sfSprite_setScale(iFight[1].sprite, scale);
    sfSprite_setPosition(iFight[1].sprite, pos);
    sfSprite_setTextureRect(iFight[1].sprite, rect);
}
