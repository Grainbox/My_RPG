...
static void draw_texts(global *all, int *dial)
{
    ...
    if (*dial == 0) {
        ...
        sfRenderWindow_drawText(WINDOW, all->game.npc.alys_talk, NULL);
    } else if (*dial == 1) {
...
void cond_speak_alys(global *all, int *dial)
{
    draw_texts(all, dial);
    increment_alys_dial(dial);
}
...
void speak_alys(global *all)
{
    ...
    while (sfRenderWindow_isOpen(all->window)) {
        ...
        if ((event = is_event(all)) == 1)
            break;
        cond_speak_alys(all, &dial);
        if (dial == 3) {
            all->inventory.letter = 0;
            break;
        }
        all->inventory.sword = 1;
        all->inventory.book = 1;
